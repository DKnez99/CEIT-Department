...
@Component({
  selector: 'app-zaposleni-predmet-projekat',
  templateUrl: './zaposleni-predmet-projekat.component.html',
  styleUrls: ['./zaposleni-predmet-projekat.component.css']
})
export class ZaposleniPredmetProjekatComponent implements OnInit {
  ...
  fileUpload(){
    ...
    file.set('file',fileBlob,this.size+'*'+this.employeeFullName+'*'+this.subject.code+'*'+'D'+'*'+fileBlob.name);
  ...
  .pipe(
  mergeMap(()=>this.fileService.getAllBySubjectAndType(this.subject.code,'D'))

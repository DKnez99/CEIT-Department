...
@Component({
  selector: 'app-zaposleni-predmet-o',
  templateUrl: './zaposleni-predmet-o.component.html',
  styleUrls: ['./zaposleni-predmet-o.component.css']
})
export class ZaposleniPredmetOComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var subject:Subject=JSON.parse(localStorage.getItem('predmet') || '{}');
    this.id=subject.id,
        ...
        this.subjectService.updateSubject(
          ...
          this.projectVisible).subscribe((subject:Subject)=>{
            if(!subject){
              ...
              localStorage.setItem('predmet', JSON.stringify(pr))
              this.errorMsg='';

...
@Component({
  selector: 'app-zaposleni-predmet-lab',
  templateUrl: './zaposleni-predmet-lab.component.html',
  styleUrls: ['./zaposleni-predmet-lab.component.css']
})
export class ZaposleniPredmetLabComponent implements OnInit {
  ...
  fileUpload(){
    ...
    file.set('file',fileBlob,this.size+'*'+this.employeeFullName+'*'+this.subject.code+'*'+'L'+'*'+fileBlob.name);
  ...
  .pipe(
  mergeMap(()=>this.fileService.getAllBySubjectAndType(this.subject.code,'L'))

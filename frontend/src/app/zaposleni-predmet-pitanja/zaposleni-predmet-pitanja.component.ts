...
@Component({
  selector: 'app-zaposleni-predmet-pitanja',
  templateUrl: './zaposleni-predmet-pitanja.component.html',
  styleUrls: ['./zaposleni-predmet-pitanja.component.css']
})
export class ZaposleniPredmetPitanjaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fileService.getAllBySubjectAndType(this.subject.code,'Q').subscribe((files:File[])=>{
      this.files=files;
  ...
  fileUpload(){
    ...
    file.set('file',fileBlob,this.size+'*'+this.employeeFullName+'*'+this.subject.code+'*'+'Q'+'*'+fileBlob.name);
  ...
  .pipe(
  mergeMap(()=>this.fileService.getAllBySubjectAndType(this.subject.code,'Q'))
